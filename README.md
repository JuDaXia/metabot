...
| **中心同步** | 跨实例同步走中心 archive（只读镜像）— 详见 [docs/internal/central-architecture.md](docs/internal/central-architecture.md) |
...
跨实例同步走中心 archive — 配置和流程详见 [docs/internal/central-architecture.md](docs/internal/central-architecture.md)。`MEMORY_INSTANCE_TOKEN` 可写当前实例兜底 namespace 以及已配置的 bot/project namespace，读共享内容；管理员 token 仍保留完整访问权限。

> **文件夹默认私有**：新建文件夹默认 `visibility=private`；通过 `mm share` 或 `PUT /api/folders/<id> { "visibility": "shared" }` 显式标记为 shared 后，才会被中心 archive 同步到其它实例。
>
> ```bash
> mm share /projects/our-team          # 让其它实例可见
...
| `METABOT_INSTANCE_ID` | 自动生成 | 当前 MetaBot 实例 ID，用作 memory namespace |
| `METABOT_CLUSTER_URL` | — | 保留给中心架构（见 [docs/internal/central-architecture.md](docs/internal/central-architecture.md)） |
...
|------|------|------|
| `GET` | `/api/health` | 健康检查 |
| `GET` | `/api/manifest` | 当前实例身份与能力 manifest |
| `GET` | `/api/bots` | 列出 Bot |
| `POST` | `/api/bots` | 运行时创建 Bot |
| `DELETE` | `/api/bots/:name` | 删除 Bot |
| `POST` | `/api/talk` | 与 Bot 对话 |
...
# MetaMemory
mm search "部署指南"                 # 本地 MetaMemory 全文搜索
mm list                             # 列出文档
mm folders                          # 文件夹树
