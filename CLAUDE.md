...
- **Model catalog**: the `/model list` picker and `GET /api/models` read from [src/engines/model-catalog.ts](src/engines/model-catalog.ts). Claude models are fetched live from the Anthropic Models API (`GET /v1/models`) when `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` is set (cached 1h), with a maintained static fallback (incl. the latest `claude-opus-4-8`); Kimi/Codex are static. Default Claude model is `claude-opus-4-8`.
- **Group-chat sender attribution**: in Feishu group chats each inbound user message is prefixed with its sender — `【群聊消息 · 发送者：<name>】` — before it reaches Claude, and a system-prompt note tells Claude to attribute each message to its sender (so it no longer assumes every message is from the owner). Names are resolved from the chat member list ([src/feishu/message-sender.ts](src/feishu/message-sender.ts) `getUserName`, cached per chat, uses the bot's existing `im` scope — **no contact scope needed**) with a short-id fallback. 1:1 chats are unchanged. Wiring: [event-handler.ts](src/feishu/event-handler.ts) resolves `senderName` → `IncomingMessage` → [message-bridge.ts](src/bridge/message-bridge.ts) `executeQuery` tags the model-facing prompt (not the user-facing card) and sets `apiContext.isGroup` → both executors emit the attribution note.
- **Reasoning effort** (Claude only): the SDK `query({ options: { effort } })` knob — `low | medium | high | xhigh | max` (higher levels are model-gated; the SDK silently downgrades unsupported ones). Resolution precedence: per-chat `/effort <level>` session override (persisted in the session store) **>** bot config (`bots.json` `claude.effort` / `CLAUDE_EFFORT` env) **>** the model's own default (when unset, MetaBot passes nothing). Defined as `EffortLevel` + `parseEffort()` in [src/engines/types.ts](src/engines/types.ts); applied in both the legacy [executor.ts](src/engines/claude/executor.ts) and the [persistent-executor.ts](src/engines/claude/persistent-executor.ts) (the latter binds it at construction, so a mid-session `/effort` change rebinds only on a fresh executor — same as `/model`).
