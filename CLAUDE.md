...
- **MetaMemory**: embedded SQLite document server at `META_MEMORY_URL` (default `http://localhost:8100`). Claude reads/writes via the `metamemory` skill; `/memory list|search|status` query directly. Each instance has a stable fallback `METABOT_MEMORY_NAMESPACE=/instances/<instanceId>`, while bots can use stable project namespaces via `memoryProject` (`/projects/<slug>`) or explicit `memoryNamespace`; `MEMORY_INSTANCE_TOKEN` can write the instance fallback plus configured bot/project namespaces while reading shared folders. Cross-instance sync runs through the central archive — see [docs/internal/central-architecture.md](docs/internal/central-architecture.md).
- **MetaMemory — default-private folders**: newly created folders default to `visibility=private`. Opt in with `mm share <folder-path>` or `PUT /api/folders/<id> { "visibility": "shared" }`; revoke with `mm unshare`. Shared folders are the ones mirrored by the central archive.
