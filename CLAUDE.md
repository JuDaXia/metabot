...
- **Audit log**: every `/memory/*` and `/api/skills/*` request appends a JSONL line to `data/audit/YYYY-MM-DD.jsonl` (`{ts, op, path, principalId, sourceIp, status, latencyMs}`; daily + 100 MB rotation; fsync per write). Disable via `METABOT_AUDIT_ENABLED=false`; relocate via `METABOT_AUDIT_DIR=<path>`. Query with `mm audit <YYYY-MM-DD> [--filter principal=X] [--filter op=X]`.
- **Central client mode** (Phase 2 pivot): `METABOT_BACKEND=central` proxies all `memory/*` and `skill-hub/*` reads/writes to a central HTTPS server set via `CENTRAL_URL` + `CENTRAL_TOKEN`. Outbound calls carry `Authorization: Bearer ${CENTRAL_TOKEN}` and an `X-MetaBot-Origin: client` header. Local SQLite is retained as a read-only fallback cache: on network failure or 5xx, reads serve from cache while writes fail loudly with HTTP 502 `central_unreachable`. Default `local` mode is unchanged. Toggle the fallback with `CENTRAL_FALLBACK_READONLY=false` (default `true`). Both `CENTRAL_URL` and `CENTRAL_TOKEN` are required when `METABOT_BACKEND=central`; the process refuses to start otherwise.
